...
from agent.Arena import Arena
from agent.MCTS import MCTS
from agent.gobang.GobangGame import GobangGame, display
from agent.gobang.GobangPlayers import *
from agent.gobang.keras.NNet import NNetWrapper as NNet
...
class RLAgent():
	def __init__(self, gomoku, current_state):
		"""
		use this script to play any two agents against each other, or play manually with
		any agent.
		"""
		self.__gomoku = gomoku
		self.__currentState = current_state
		self.g = GobangGame()

		# all players
		self.hp = HumanGobangPlayer(self.g).play

		# nnet players
		self.n1 = NNet(self.g)
		self.n1.load_checkpoint('../data/','checkpoint_12.pth.tar')
		...
		self.arena = Arena(self.n1p, self.hp, self.g, display=display)
		#print(arena.playGames(2, verbose=True))
		self.arena_board = self.g.getInitBoard()

	def one_step(self):
		(i, j), self.arena_board = self.arena.play_game_retro(self.arena_board, np.reshape(self.arena_board, (self.g.n, self.g.n)))
